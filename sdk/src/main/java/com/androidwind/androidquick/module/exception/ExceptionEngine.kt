    ...
    fun handleException(throwable: Throwable): ApiException {
        ...
        if (throwable is HttpException) {             //HTTP错误
            ...
            when (throwable.code()) {
                ...
                GATEWAY_TIMEOUT -> ex.message = "服务器没有从远端服务器得到及时的响应"
                INTERNAL_SERVER_ERROR -> ex.message = "服务器内部错误"
                BAD_GATEWAY -> ex.message = "服务器接收到远端服务器的错误响应"
                SERVICE_UNAVAILABLE -> ex.message = "服务器维护或者过载，服务器当前无法处理请求"
                else -> ex.message = "网络错误"  //其它均视为网络错误
            }
        } else if (throwable is ServerException) {    //服务器返回的错误
            ex = ApiException(throwable, throwable.code)
            ex.message = throwable.message
